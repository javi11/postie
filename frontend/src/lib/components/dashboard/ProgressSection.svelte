<script lang="ts">
  import { Card, Heading, P, Progressbar, Button } from "flowbite-svelte";
  import { CloseCircleSolid } from "flowbite-svelte-icons";
  import { progress, isUploading } from "$lib/stores/app";
  import { toastStore } from "$lib/stores/toast";
  import { CancelJob, CancelUpload } from "$lib/wailsjs/go/main/App";
  import { formatTime, formatSpeed } from "$lib/utils";
  ...
  // Cancel current job function
  async function cancelCurrentJob() {
    if (!progressData.jobID) {
      toastStore.error("Cannot cancel", "No job ID available");
      return;
    }

    try {
      await CancelJob(progressData.jobID);
      toastStore.success(
        "Job cancelled",
        "Upload has been cancelled successfully"
      );
  ...
  <div class="space-y-6">
    <div class="flex items-center justify-between">
      ...
      <div class="flex items-center gap-3">
        ...
        >
          <div
            class="w-2 h-2 rounded-full transition-all duration-300 {uploading
      ...
      <div class="space-y-6">
        ...
        <div class="space-y-3">
          <div class="flex justify-between items-center">
            ...
            >
              {progressData.completedFiles}/{progressData.totalFiles} files ({Math.round(
        ...
        {#if progressData.currentFile}
          ...
          >
            <div class="flex justify-between items-center">
              ...
              >
                {Math.round(progressData.currentFileProgress)}%
              </span>
            </div>
            <P
              class="text-sm text-blue-700 dark:text-blue-300 font-medium truncate"
              title={progressData.currentFile}
            >
              {progressData.currentFile}
            </P>
            <Progressbar
              progress={progressData.currentFileProgress}
              color="blue"
              size="h-2"
              class="w-full"
            />
          ...
          <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
            ...
            >
              ...
              <span
                ...
                >{formatTime(progressData.elapsedTime * 1000)}</span
              >
            </div>
            ...
            {#if progressData.speed > 0}
              ...
              >
                ...
                <span
                  ...
                  >{formatSpeed(progressData.speed * 1024)}</span
                >
              </div>
            {/if}

            {#if progressData.secondsLeft > 0}
              <div
              ...
              >
                ...
                <span
                  ...
                  >{formatTime(progressData.secondsLeft * 1000)}</span
                >
              </div>
            {/if}
          </div>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          >
            <span class="text-sm text-gray-600 dark:text-gray-400">Stage:</span>
            <span class="text-sm font-medium text-gray-900 dark:text-white"
              >{progressData.stage}</span
            >
          </div>
        ...
        >
          ...
          >
          <span class="text-sm font-medium text-gray-900 dark:text-white">
            {new Date(progressData.lastUpdate * 1000).toLocaleTimeString()}
          </span>
        </div>
      </div>
    {:else}
