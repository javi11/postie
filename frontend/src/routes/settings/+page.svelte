<script lang="ts">
  ...
  onMount(async () => {
    ...
    const unsubscribe = appStatus.subscribe((status: any) => {
  ...
  async function handleSaveConfig() {
    try {
      ...
      configToSave.servers = configToSave.servers.map((server: any) => ({
        ...server,
        port: parseInt(server.port) || 119,
        max_connections: parseInt(server.max_connections) || 10,
        max_connection_idle_time_in_seconds:
          parseInt(server.max_connection_idle_time_in_seconds) || 300,
        max_connection_ttl_in_seconds:
          parseInt(server.max_connection_ttl_in_seconds) || 3600,
      }));

      // Convert posting fields
      configToSave.posting.max_retries =
        parseInt(configToSave.posting.max_retries) || 3;
      configToSave.posting.article_size_in_bytes =
        parseInt(configToSave.posting.article_size_in_bytes) || 750000;
      ...
      configToSave.par2.volume_size =
        parseInt(configToSave.par2.volume_size) || 153600000;
      configToSave.par2.max_input_slices =
        parseInt(configToSave.par2.max_input_slices) || 4000;

      // Convert watcher fields
      if (configToSave.watcher) {
        configToSave.watcher.size_threshold =
          parseInt(configToSave.watcher.size_threshold) || 104857600;
        configToSave.watcher.min_file_size =
          parseInt(configToSave.watcher.min_file_size) || 1048576;
